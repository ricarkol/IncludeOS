...
void *lu_function(void *_arg)
{
	static int t = 41;
	t++;
	struct luargs *args = (struct luargs *) _arg;
	int bi = args->bi;
	int bj = args->bj;
	int nbi = args->nbi;
	int nbj = args->nbj;
	...
	while (!alldone(nbi, nbj)) {
		for (i = 0; i < nbi; ++i) {
			for (j = 0; j < nbj; ++j) {
				...
				status[i][j] = RUNNING;
#if USE_FORK
				int r = pthread_create(&threads[nbj * i + j], NULL,
                                                       lu_function, &args[i][j]);
				assert(r == 0);
#endif
			}
		}
...
#if USE_FORK
  				void* retval = NULL;
				pthread_join(threads[nbj * i + j], &retval);
				//printf("Thread xxx returned: %ld \n", (long)retval);
#endif
				status[i][j] = DONE;
			}
		}
